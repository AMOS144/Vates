"""合并路径消融:验证"动态深度 + pos0 救回"能否叠加。

对比 fixed-K3 / tree-pos0(救回单开) / adaptive(动态深度单开) / adaptive+rescue(合并)。
关键看:adaptive+rescue 的 tok/s 是否 > adaptive 单开(叠加为正),以及是否 > tree-pos0 单开。
avg_accept_len 近确定性看接受长度变化;lossless 用逐轮 vs fixed 首轮最大失配(应为 0)。
配置(生产,slots=32,depth_max=3 → 无 cap 溢出):
  STREAM_BLOB_LOADER=1 NATIVE_FUSED_PREFETCH=1 EXPERT_SLOTS=32 ZEROCOPY_DUAL_SOURCE=1 \
  SIDEREGION_LFU=1 POOL_SPEC_SLOTS=32 K=3 .venv/bin/python benchmarks/_bench_merged.py
"""
import json
import os
...
TAU = os.environ.get("MTP_CONF_TAU", "0.3")
...
# (名称, TREE_TOP2, TREE_TOP2_P1, ADAPTIVE, ADAPTIVE_RESCUE)
CONFIGS = [
    ("fixed-K3",      "0", "0", "0", "0"),
    ("tree-pos0",     "1", "0", "0", "0"),
    ("adaptive",      "0", "0", "1", "0"),
...
def _set(cfg):
    ...
    os.environ["MTP_ADAPTIVE_RESCUE"] = ar
    os.environ["MTP_CONF_TAU"] = TAU
    os.environ["MTP_DEPTH_MAX"] = "3"
...
def main():
    ...
    for p in PROMPTS:
        ...
        for c in CONFIGS:
            ...
            rec[name] = {"tps": median(tps_runs), "acc": acc, "mm": mm, "resc": resc}
        rows.append(rec)
        print(json.dumps(rec, ensure_ascii=False), flush=True)

    print(f"\n=== 合并路径消融汇总(6 prompt 平均, tau={TAU}, depth_max=3)===")
    print(f"{'config':>12} {'acc_len':>8} {'tps_med_avg':>12} {'resc':>6} {'max_mm':>7} {'vs_fixed':>9}")
    base = sum(r["fixed-K3"]["tps"] for r in rows) / len(rows)
    for name in [c[0] for c in CONFIGS]:
        ...
        d = (tps / base - 1) * 100
        print(f"{name:>12} {acc:>8.3f} {tps:>12.2f} {resc:>6} {mm:>7} {d:>+8.2f}%")

    a_ad = sum(r["adaptive"]["tps"] for r in rows) / len(rows)
    a_ar = sum(r["adap+rescue"]["tps"] for r in rows) / len(rows)
    print(f"\n叠加检验: adap+rescue vs adaptive 单开 = {(a_ar/a_ad-1)*100:+.2f}% "
          f"(>0 才说明救回在动态深度上还能再叠加收益)")

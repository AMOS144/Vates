"""全模型 clip-search(scale-only)from FP16:逐组最优裁剪量化成 2-bit g128(无 Hessian、无校准)。
...
from mlx_streaming.prep.clip_search_quant import clip_search_quantize

FP16_DIR = os.environ.get("FP16_DIR", "/tmp/qwen3_next_80b_fp16")
OUT_DIR = os.environ.get("OUT_DIR", os.path.abspath("models/qwen3_next_experts_fp16clip_g128"))
...
def main():
    ...
    for si, shard in enumerate(sorted(shard_keys)):
        ...
        for (L, E) in list(pending):
            if len(pending[(L, E)]) == 3:
                out = {}
                for proj in PROJS:
                    q, s, b = clip_search_quantize(pending[(L, E)][proj], group_size=GROUP, bits=BITS)
    ...
    meta["requantized_from"] = {"dir": FP16_DIR, "bits": "bf16",
                                "method": "clip_search_scale_only", "single_pass_from_fp16": True}
